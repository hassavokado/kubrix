...
With the scaffolder [template for multi-stage app with kargo pipeline](https://backstage.kubrix.digiorg.cloud/create/templates/default/multi-stage-app-with-kargo-pipeline) you can automatically create such a repo with the predefined example application 'podtato-head'.

![image](../../img/app-onboarding-1.png)

Just specifiy your application name and a description and be sure that the "FQDN" points to `kubrix.digiorg.cloud`. Then click 'Next'.
