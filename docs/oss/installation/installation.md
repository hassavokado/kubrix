...
### Create Github OAuth App and set secrets in vault
  ...
  **For Kubernetes Cluster**:  
  ...
  - Homepage URL: `https://backstage.kubrix.digiorg.cloud`
  - Authorization callback URL: `https://backstage.kubrix.digiorg.cloud/api/auth/github`
...
| -------- | ------- | ------- | ------- |
| Backstage  | https://backstage.kubrix.digiorg.cloud | via github | via github |
| ArgoCD | https://argocd.kubrix.digiorg.cloud| admin | `kubectl get secret -n argocd argocd-initial-admin-secret -o=jsonpath='{.data.password}' \| base64 -d` |
| Kargo | https://kargo.kubrix.digiorg.cloud     | admin | - |
| Grafana    | https://grafana.kubrix.digiorg.cloud | `kubectl get secret -n grafana grafana-admin-secret -o=jsonpath='{.data.userKey}' \| base64 -d` | `kubectl get secret -n grafana grafana-admin-secret -o=jsonpath='{.data.passwordKey}' \| base64 -d` |
| Keycloak    | https://keycloak.kubrix.digiorg.cloud | admin | `kubectl get secret -n keycloak keycloak-admin -o=jsonpath='{.data.admin-password}' \| base64 -d` |
| FalcoUI    | https://falco.kubrix.digiorg.cloud | `kubectl get secret -n falco falco-ui-creds -o=jsonpath='{.data.FALCOSIDEKICK_UI_USER}' \| base64 -d \| awk -F: '{print $1}'` | `kubectl get secret -n falco falco-ui-creds -o=jsonpath='{.data.FALCOSIDEKICK_UI_USER}' \| base64 -d \| awk -F: '{print $2}'` |
